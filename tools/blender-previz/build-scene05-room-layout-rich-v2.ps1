...
$outputDir = Join-Path $packageDir 'previz\room-layout-rich-v2'
$blendFile = Join-Path $outputDir 'scene05-room-layout-rich-v2.blend'
$reportFile = Join-Path $outputDir 'evaluation-room-layout-rich-v2.json'
$placementFile = Join-Path $outputDir 'room-layout-rich-plan-v2.md'
$contactSheet = Join-Path $outputDir 'room-layout-rich-contact-sheet-v2.jpg'
...
if ($LASTEXITCODE -ne 0) { throw "SCENE 5 rich room-layout build failed with exit code $LASTEXITCODE" }

$report = Get-Content -LiteralPath $reportFile -Raw -Encoding UTF8 | ConvertFrom-Json
if (-not $report.passed -or -not $report.room_layout_mode -or -not $report.rich_room_mode) {
    throw "SCENE 5 rich room-layout evaluation failed. See $reportFile"
}
if ($report.rich_room_details.characters_modified -or
    ...
    throw "SCENE 5 rich-room identity or placement evaluation failed. See $reportFile"
}

& $ffmpeg.Source -hide_banner -loglevel error -y -framerate 1 -start_number 1 -i (Join-Path $outputDir 'layout_%04d.png') `
    -vf "scale=533:-1,tile=3x1:nb_frames=3" -frames:v 1 -update 1 $contactSheet
if ($LASTEXITCODE -ne 0) { throw "Rich room-layout contact sheet failed with exit code $LASTEXITCODE" }
