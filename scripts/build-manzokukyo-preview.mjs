...
export async function buildManzokukyoPreview() {
  ...
  const html = (await readFile(path.join(source, 'index.html'), 'utf8')).replaceAll('{{TIKTOK_URL}}', escapeHtml(tiktok.url)).replaceAll('{{GAME_LINKS}}', gameLinks);
  await writeFile(path.join(output, 'index.html'), html, 'utf8');
  for (const name of ['styles.css', 'site.js', 'corridor.js', 'audio.js', 'route.js', 'guestbook-adapter.js']) await cp(path.join(source, name), path.join(output, name));
