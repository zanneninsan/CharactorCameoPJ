...
const bust = giant.getObjectByName('Darenin head shoulders and chest');
assert.ok(bust.geometry.attributes.position.count > 300 && bust.geometry.attributes.position.count < 16000, 'bust is real model geometry and stays lightweight');
assert.equal(bust.isSkinnedMesh, undefined, 'the giant uses no extra skinning');
assert.ok(bust.geometry.boundingBox.min.y >= -.00001, 'the chest crop has a flat base');
assert.ok(bust.geometry.boundingBox.getSize(new T.Vector3()).y > .5, 'shoulders and chest are retained below the head');
crowd.setAnomaly({ kind: 'giant-darenin' });
const initialBust = bust.rotation.clone();
...
assert.ok(giantBounds.max.y < 6.4 && Math.abs(giantBounds.min.y) < .001 && giantBounds.min.x > -5.2 && giantBounds.max.x < 5.2, 'the bust is grounded and stays below the ceiling and inside the walls');
const still = bust.matrixWorld.clone();
for (let i = 0; i < 20; i++) crowd.update(.05, { x: 0, z: -30 }, { paused: true });
scene.updateMatrixWorld(true); assert.ok(bust.matrixWorld.equals(still), 'modal pauses the giant');
crowd.setAnomaly({ kind: 'giant-darenin' });
crowd.update(.05, { x: 0, z: -30 }, { reduced: true }); scene.updateMatrixWorld(true);
const reducedPose = bust.matrixWorld.clone();
for (let i = 0; i < 20; i++) crowd.update(.05, { x: 0, z: -30 }, { reduced: true });
scene.updateMatrixWorld(true); assert.ok(giant.visible && bust.matrixWorld.equals(reducedPose), 'reduced motion retains the clue without swaying');
crowd.setAnomaly(anomaly); assert.equal(giant.visible, false, 'switching to the rush also removes the pedestal');
crowd.setAnomaly({ kind: 'giant-darenin' }); assert.ok(bust.rotation.equals(initialBust), 'reselecting the giant starts with a clean pose');
...
let geometryDisposals = 0, finishDisposals = 0, bustDisposals = 0;
const bustResources = new Set(); giant.traverse(object => { if (object.geometry) bustResources.add(object.geometry); if (object.material && object.material !== skins[0].material) bustResources.add(object.material); });
for (const resource of bustResources) resource.addEventListener('dispose', () => bustDisposals++);
...
assert.equal(bustDisposals, bustResources.size, 'bust and pedestal resources are each released once');
