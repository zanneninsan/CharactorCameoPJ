...
export function renderGallery3DExperience(character, { htmlPage, escapeHtml, assetVersionQuery, alias = false }) {
  ...
  const stage = `<section class="gallery-3d-stage" id="gallery-3d-exhibition" aria-labelledby="gallery-3d-title">
        ...
        <p class="gallery-visitor-note">巡回中：誰念院さん（たぶん）。歩き回っていても異変ではありません。</p>
        <div class="gallery-3d-controls" aria-label="画廊を歩く"><button type="button" data-gallery-visitor-toggle aria-pressed="true" title="誰念院さんの表示と散策を切り替える">誰念院さん ON</button><button type="button" data-gallery-3d-prev aria-label="前の記録へ">← 前へ</button><div class="gallery-3d-position" aria-live="polite"><small data-gallery-3d-room>展示室 I</small><strong data-gallery-3d-record>ARCHIVE 01</strong></div><button type="button" data-gallery-3d-next aria-label="次の記録へ">次へ →</button><button type="button" data-gallery-3d-inspect>この記録を開く</button><button type="button" data-gallery-3d-overview>展示室を見渡す</button><button type="button" data-gallery-3d-list aria-controls="gallery-3d-catalog" aria-expanded="true">一覧を閉じる</button></div>
      </section>
