...
export function renderGallery3DExperience(character, { htmlPage, escapeHtml, assetVersionQuery, alias = false }) {
  ...
  const stage = `<section class="gallery-3d-stage" id="gallery-3d-exhibition" aria-labelledby="gallery-3d-title">
        ...
        <p class="gallery-visitor-note" data-gallery-visitor-note role="status"></p>
  ...
  const loopPanel = `<div class="gallery-loop-ui" data-gallery-loop-ui hidden>
        <div class="gallery-loop-modes" role="group" aria-label="画廊の遊び方"><button type="button" data-gallery-loop-mode="loop" aria-pressed="true">異変と検印の回廊</button></div>
        <div class="gallery-loop-panel" data-gallery-loop-panel><details class="gallery-loop-rule"><summary>遊び方</summary><div><strong>異変があれば、引き返せ。</strong><p data-gallery-loop-hint>まずは正常な24枚を覚えて、奥の扉へ。</p><p>左のボタンで移動、ドラッグで見回す。絵をタップして拡大・検印。</p></div></details><div class="gallery-loop-count"><small data-gallery-loop-round>初回 / 正常な展示</small><span>連続正解 <b data-gallery-loop-streak>00</b><small>最高 <b data-gallery-loop-best>00</b></small></span></div><button type="button" data-gallery-loop-restart title="検印と連続正解をリセットして最初から遊ぶ">最初から</button></div>
  ...
  const transition = `<div class="gallery-loop-transition" data-gallery-loop-transition role="status" hidden><div class="gallery-loop-caught" aria-hidden="true">${Array.from({ length: 4 }, () => '<i><b></b><b></b></i>').join('')}</div><div class="gallery-loop-eye" aria-hidden="true"><i></i></div><span>MANZOKUKYO / RECURSION</span><strong data-gallery-loop-transition-title></strong><p data-gallery-loop-transition-note></p><button type="button" data-gallery-loop-retry hidden>展示を読み込み直す</button></div>`;
  const exhibition = stage.replace('<div class="gallery-3d-viewport">', '<div class="gallery-3d-viewport">' + loopPanel + transition + '<div class="gallery-loop-touch" data-gallery-loop-touch role="status" hidden><span>……つかまえた。</span></div>');
  let body = original.body.slice(0, introStart) + exhibition + original.body.slice(puzzleStart);
  body = body.replace('<div><span class="gallery-eyebrow">Sealed records', '<div><a class="gallery-3d-return" href="#gallery-3d-exhibition">↑ 画廊へ戻る</a><br><span class="gallery-eyebrow">Sealed records');
  ...
  body += `<dialog class="gallery-loop-inspection" data-gallery-loop-inspection aria-labelledby="gallery-loop-inspection-title"><div><img alt="展示中の記録"><aside class="gallery-seal-imprint" data-gallery-loop-imprint hidden role="status" aria-live="polite"><div class="gallery-seal-rays" aria-hidden="true"></div><div class="gallery-seal-orbit" aria-hidden="true"></div><div class="gallery-seal-dust" aria-hidden="true">${Array.from({ length: 24 }, (_, i) => `<i style="--i:${i}"></i>`).join('')}</div><div class="gallery-seal-mark"><span>満 足 教</span><strong data-gallery-loop-imprint-fragment></strong><small>検 印</small></div><p data-gallery-loop-imprint-number></p><em>印は、あなたの手に。</em></aside><footer><strong id="gallery-loop-inspection-title" data-gallery-loop-inspection-title></strong><button type="button" data-gallery-loop-collect hidden>⊹ 検印を仮押しする</button><button type="button" data-gallery-loop-inspection-close>展示室へ戻る</button><p data-gallery-loop-seal-note role="status"></p></footer></div></dialog>`;
  ...
  const achievements = galleryDebugOptions.filter(option => option.kind !== 'normal');
  body = body.replace('異変と検印の回廊</button></div>', `異変と検印の回廊</button><button type="button" data-gallery-achievements-open>実績 0 / ${achievements.length}</button></div>`);
  body += `<dialog class="gallery-achievements" data-gallery-achievements aria-labelledby="gallery-achievements-title"><header><span>ANOMALY ARCHIVE</span><h2 id="gallery-achievements-title" tabindex="-1" autofocus>異変の実績ボード</h2><p data-achievement-count>遭遇 0 / ${achievements.length}　看破 0 / ${achievements.length}</p></header><p>周回の判定で遭遇を記録。正しく引き返すと看破になります。未遭遇の異変は秘密です。</p><div class="gallery-achievement-grid">${achievements.map((_, index) => `<article data-achievement-card data-level="0"><span>異変 ${String(index + 1).padStart(2, '0')}</span><strong>？？？</strong><small>未遭遇</small></article>`).join('')}</div><p class="gallery-achievement-complete" data-achievement-complete hidden>全異変を看破しました。<br>あなたの観察を、満足教は記憶しました。</p><p class="gallery-achievement-note">このブラウザに保存されます。「最初から」でも実績は残ります。デバッグ・作品鑑賞は対象外です。</p><div class="gallery-achievement-actions"><button type="button" data-achievements-close>画廊へ戻る</button><button type="button" data-achievements-viewing>作品鑑賞</button></div><section class="gallery-viewing-confirm" data-viewing-confirm hidden aria-labelledby="gallery-viewing-warning"><h3 id="gallery-viewing-warning">作品鑑賞に切り替えますか？</h3><p>連続正解が 0 に戻り、仮押し中の印は消えます。持ち帰り済みの検印・最高記録・実績は残ります。</p><div class="gallery-achievement-actions"><button type="button" data-viewing-cancel>キャンセル</button><button type="button" data-viewing-accept>記録をリセットして作品鑑賞へ</button></div></section></dialog>`;
