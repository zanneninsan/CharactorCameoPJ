...
assert.equal(book.snapshot().total, 15); assert.ok(book.snapshot().entries.every(entry => entry.label === '？？？'));
...
assert.equal(trigger.textContent, '実績 0 / 15'); assert.ok(cards.every(card => card.children.strong.textContent === '？？？'));
...
console.log('Achievements passed: all fifteen hidden slots, encounter/solve, persistence/merge/corruption/storage denial, debug exclusion, visible completion and modal teardown.');
