...
assert.equal(book.snapshot().total, 19); assert.ok(book.snapshot().entries.every(entry => entry.label === '？？？'));
...
console.log('Achievements passed: all nineteen hidden slots, encounter/solve, persistence/merge/corruption/storage denial, debug exclusion, visible completion and modal teardown.');
