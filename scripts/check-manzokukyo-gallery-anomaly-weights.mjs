...
const weighted = distribution({ visitorsReady: true, encounteredKinds }, 15500);
...
for (const known of [[], allKinds]) {
  const uniform = distribution({ visitorsReady: true, encounteredKinds: known }, 15000);
  assert.equal(uniform.size, 15); assert([...uniform.values()].every(count => count === 1000), 'fresh and completed collections are uniform');
}
const withoutVisitors = distribution({ visitorsReady: false, encounteredKinds }, 10000);
assert.equal(withoutVisitors.size, 10);
...
assert.deepEqual(distribution({ visitorsReady: true, encounteredKinds: [...encounteredKinds, ...encounteredKinds, 'unknown'] }, 15500), weighted, 'duplicate and stale saved names do not affect weights');
